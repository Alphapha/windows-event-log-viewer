...
5. 使用级别过滤器（全部级别下拉列表）筛选特定级别的事件
6. 点击「🔄 重建」按钮可强制重新构建数据库（如果数据异常或损坏时使用）
...
GET /api/events?filename=Application.evtx&levels=错误,警告  # 按级别过滤
```
...
**解决方法**：
1. 在顶部工具栏找到"全部级别"下拉列表
2. 选择要过滤的级别（如"错误"）
3. 页面会自动刷新，只显示该级别的事件
4. 再次选择"全部级别"可恢复显示所有事件
5. 级别过滤同样适用于搜索功能

...
- 📁 **文件浏览**：左侧显示evtx目录下所有EVTX日志文件，支持收起/展开侧边栏
- 📊 **日志查看**：右侧以表格形式展示事件日志全部内容
- 📈 **事件ID统计**：点击左侧文件项可展开显示该文件的事件ID统计，支持点击筛选
- 🎯 **字段选择**：可自定义选择要显示的字段列
...
#### 删除文件
- 左侧文件列表右侧常驻显示操作按钮（📦归档、🗑️删除）
- 点击🗑️按钮直接删除文件（**无需确认，不可恢复**）
- 删除会同时移除对应的SQLite数据库文件

#### 归档文件
- 点击📦按钮直接将文件归档到 `evtx_archive/` 目录（**无需确认**）
...
### v3.6.0 (2026-04-28)
- 🔽 **侧边栏收起**：新增侧边栏收起/展开功能，点击按钮可隐藏侧边栏
- ️ **移除分类筛选**：移除日志类型分类筛选下拉列表，文件列表平铺显示
- 🎨 **布局优化**：文件项采用两行布局，避免文件名、标签、按钮重叠
-  **搜索优化**：空搜索时自动回退到加载全部内容
- ⚡ **操作简化**：归档/恢复/删除操作无需确认，直接执行
- 📝 **文档完善**：更新使用方法和API文档
